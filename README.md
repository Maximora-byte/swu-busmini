...
新增彩色路网实验预览与一次换乘查询。启动时正式地图仍遵循上述审核门禁；点击线路编号后，仅展示所选线路当前方向的离线腾讯参考片段并自动调整视野。参考虚线与正式数据完全分开，不表示真实校车路径已确认，可关闭“腾讯参考”隐藏。
...
当前已录入 1～9 路第一版，但不将其声明为 2026 年官方实时线路。2026-09-22 用户确认“中图”为中心图书馆，已保留 zhongtu ID 并关联 library POI。实际候车位置及当年运营方向仍需现场核实。项目不接入未经授权的实时公交接口。
...
例如二食堂关联同名站点后可以查询到 4 路；中心图书馆根据用户确认关联 zhongtu，可查询 9 路，“中图”“图书馆”“西大图书馆”作为搜索别名。当前 POI 坐标、真实步行距离和上下车位置仍需后续现场校验。
...
灵活校园车结果会提示“该线路支持沿途停靠，请结合现场情况确认上下车位置”，不承诺一定停靠。当前只计算静态线路连通性，不计算 GPS 或步行距离、不判断最近站点，也不代表实时车辆状态。现在另有一次换乘查询，见下节。

## 彩色路网实验预览与一次换乘

地图提供“路网总览 / 腾讯参考”开关和“看全线”。1～9 路沿用 2025 图片色系。点击任一线路编号、方向按钮或导航结果，会关闭总览、开启参考图层，并且**只显示该线路当前方向**，自动调整地图视野；用户位置 marker 不受影响。方向入口始终可见，详情可展开查看站序与缺失片段。只有手动开启“路网总览”才同时显示其他线路的首方向。

启动时仍只展示 verified 正式轨迹；选线或主动开启“腾讯参考”后，以**虚线**显示离线腾讯驾车推荐片段。地图下方显示当前线路、方向及已生成/应有路段数，防止把不完整轨迹误认为完整线路。

**实验片段不是已经确认的校车行驶路径，不能用来确认候车位置。** 图片只核对站序与线路识别色，没有地理配准基准，不能保证与腾讯底图几何重合，也不会按像素推算 GPS。缺失或可疑坐标的相邻路段留空，不补直线。全部正式站点坐标仍保持原样，未经审核的候选不进入 marker / 最近站点 / 正式轨迹仓库。

开发阶段可运行（默认全部 9 路，最多 80 次请求，硬上限 120 次）：

```bash
npm run generate:preview -- --max-requests=80
# 可仅生成指定线路；输出会替换实验数据集，不影响正式数据
npm run generate:preview -- route_1 --max-requests=30
# 只检查腾讯返回的地点标题，不改动地图预览
npm run generate:preview -- --inspect-places --max-requests=30
# 已有完整缓存时零网络请求重放
npm run generate:preview -- --max-requests=0
npm run check:data-sync
npm run validate:data
```

本地 Key 仍在被忽略的 `config.local.ts`。生成默认使用腾讯北碚区地点搜索，要求西南大学上下文与唯一、规范化后精确站名匹配，保留地点标题作为来源证据；没有唯一匹配时留空，不回退到模糊地理编码第一项。地点搜索与有方向驾车段使用 `.cache/route-preview-cache.json`，相同查询优先读缓存；请求节流、超时、有上限，不记录 Key、请求 URL 或外部原始错误。网络/API/额度错误会保留原预览。产物为独立的 `route-preview.json / .ts`，含候选、片段、生成时间与警告，始终 `verified:false / needs_review`；候选 confidence 仅反映名称匹配，不表示站点位置真实可信。运行时仅通过 Repository 读取 TS Adapter，不调用腾讯路线 API。

2026-09-25 使用地点标题证据重新采集：22 次地点搜索和 40 次驾车请求，16 个唯一地点候选，生成 **94/118 个有方向路段记录**（共享路段在不同线路中分别引用，并非 94 条独立道路）。修正了早期地理编码中六号门、26 教、外语院等候选偏差；圆顶、西师街、二食堂、大礼堂仍缺可靠匹配，不生成相邻路段。3/6/8 路参考片段数量完整，但所有路线仍待实际乘车核验。见 [本次图片与腾讯结果比对](docs/tencent-route-comparison-2026-09-25.md)；[9 月 22 日记录](docs/route-preview-trial-2026-09-22.md)仅作为历史实验保留。

在线路导航中选起终点后，先显示直达，再列出至多 6 个一次换乘候选。只在**同一 stopId** 换乘，必须是两条不同线路且两段方向有效、非零长度；明确闭合环线最多绕一圈。结果按参考站序段数和稳定 ID 排序，不承诺最短距离、最快或可安全换乘。示例：二食堂 → 中心图书馆，可查询 4 路接 9 路；每段可单独打开对应方向与地图。换乘站是否同侧、实际是否停靠、道路是否可安全穿越，仍需现场确认；不提供步行连接、两次以上换乘、实时车辆或时刻表。
...
- `/ws/place/v1/search`：仅供开发脚本获取带名称证据的候选地点；不替换校园 POI；
...
顶部提供“校园地图 / 线路导航”导航栏。在线路导航中可选择已知校园地点和参考站点：不指定起点时查询途经线路，指定起点时调用 OD Service 匹配直达线路及方向。每个结果显示上下车参考点、途经顺序、灵活停靠提示；点击“查看此方向与地图”会选中相应线路与方向。没有关联、没有直达线路或起终点相同会显示明确说明，切换地点会清除旧结果。

可复现示例：目的地选择“中心图书馆”可查询 9 路两个图示方向；起点选择“五号门”，优先显示参考站序较短的“图示顺序”，途经五号门 → 橘园 → 梅园 → 中心图书馆；环线反方向绕行也可匹配。该功能是基于参考站序的线路导航，尚不是基于真实轨迹的逐路口导航。

2026-09-22 已实测腾讯地理编码和逆地理编码，[坐标核对记录](docs/coordinate-check-2026-09-22.md)保留结果与疑点。可运行 `npm run geocode "重庆市北碚区西南大学中心图书馆" -- --verify` 复核，脚本不会写数据或自动批准候选。地址相似度不能代替站点位置审核。
...
2026-09-22 再次对照原图核对，1～9 路已录入站序未发现差异，4 路第二站仍为“二食堂”；随后用户确认 9 路“中图”为中心图书馆，已同步地点关联。没有根据图片新增坐标、推算校园轮廓或将 2025 图中的运营时间用作当前时刻表。

微信开发者工具/真机验收：打开后检查校园默认视野；切换校园/城区并拖动后再次点击回到校园；模拟外地位置确认镜头不会跳走；左右滑动选择 1～9 路，确认地图仅保留所选线路色的当前方向虚线并调整视野。切换方向、从总览再次选线、从导航结果打开方向，均应清除其他线路；重点检查 6/9 路摘要和重复站标签。展开详情可查看缺失片段，关闭“腾讯参考”后因没有 verified 轨迹而不显示线路属于预期行为。
