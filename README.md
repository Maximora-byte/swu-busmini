...
项目已完成 **Phase 4.5：1～9 路静态线路数据第一版录入**。当前版本可以获取并显示用户位置，在 1～9 路之间切换，并查看图示方向和纵向站序。Repository 只允许 `verified: true` 的 GCJ-02 坐标进入地图 marker。
...
6. 结合 2026 年站牌、通知和实际乘车复核线路数据；
7. 建立带别名的校园 POI 搜索；
8. 实现单线路直达推荐；
9. 接入上车前、下车后的步行路线；
10. 增加路线排序、时刻表和多方案；
11. 稳定后迁移可变数据到 CloudBase；
12. 仅在获得正式授权接口后接入实时车辆。
