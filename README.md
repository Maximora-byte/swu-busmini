...
新增彩色路网实验预览与一次换乘查询。默认正式地图仍遵循上述审核门禁；手动开启“实验预览”可检查离线腾讯候选片段，它与正式数据完全分开，不表示真实校车路径已确认。
...
地图新增“全部线路 / 实验预览”开关和“看全线”。1～9 路沿用 2025 图片色系；选中线加粗，关闭“全部线路”可单看一条，避免重叠遮挡。默认仍只展示 verified 正式轨迹；主动开启实验预览后，以**虚线**显示离线腾讯驾车推荐片段。未选中线路显示图示首方向，选中线路可切换方向。
...
本地 Key 仍在被忽略的 `config.local.ts`。地理编码与有方向驾车段使用 `.cache/route-preview-cache.json`，相同查询优先读缓存；请求节流、超时、有上限，不记录 Key、请求 URL 或外部原始错误。产物为独立的 `route-preview.json / .ts`，含候选、片段、生成时间与警告，始终 `verified:false / needs_review`。运行时仅通过 Repository 读取 TS Adapter，不调用腾讯路线 API。

2026-09-22 实测使用 54 次请求：20 个候选中 16 个通过基础预览过滤，生成 62 个带方向的路段记录（共享路段在不同线路中分别引用，并非 62 条独立道路）。二号门、圆顶、五号门、二食堂返回相同坐标，被排除；其余结果仍未人工审核。详情见 [实验记录](docs/route-preview-trial-2026-09-22.md)。
