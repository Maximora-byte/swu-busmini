...
项目已完成 **Phase 6：校园 POI 数据模型与线路关联**。当前版本可以获取并显示用户位置，在 1～9 路之间切换并查看图示方向和纵向站序；线路模型可区分固定站点与允许沿途停靠的校园车。新增的校园地点知识层支持按名称或别名搜索地点，并通过已登记的关联站点查询可到达线路。独立验证器、CLI 与 GitHub Actions 会持续检查线路、来源和 POI 数据质量。
...
9. 结合 2026 年站牌、通知和实际乘车复核线路及 POI 关系；
10. 实现单线路直达推荐；
11. 接入上车前、下车后的步行路线；
12. 增加路线排序、时刻表和多方案；
