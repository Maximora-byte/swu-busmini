...
项目已完成 **Phase 8：起点-终点路线匹配**。当前版本可以获取并显示用户位置，在 1～9 路之间切换并查看图示方向和纵向站序；校园地点知识层支持按名称或别名搜索地点，并通过已登记的关联站点推荐可到达线路。OD 路线匹配会进一步检查起点站与终点站在每个方向中的先后顺序，返回可直达线路、匹配方向及灵活停靠提示。独立验证器、CLI 与 GitHub Actions 会持续检查线路、来源和 POI 数据质量。
...
11. 结合 2026 年站牌、通知和实际乘车复核线路及 POI 关系；
12. 接入上车前、下车后的步行路线；
13. 增加路线排序、时刻表和多方案；
14. 稳定后迁移可变数据到 CloudBase；
15. 仅在获得正式授权接口后接入实时车辆。
...
命令会逐条输出线路与 POI 检查结果，并以退出码 `0` 表示全部通过、退出码 `1` 表示存在数据错误，可直接用于 CI。除既有线路、站点和来源规则外，POI ID 必须唯一，别名不能为空，分类与数据状态必须合法，所有 `relatedStopIds` 必须引用已存在站点。
