...
项目已完成 **Phase 5：校车线路数据质量验证系统**。当前版本可以获取并显示用户位置，在 1～9 路之间切换并查看图示方向和纵向站序；新增的独立验证器与 CLI 会检查线路引用、重复站点、环线结构、方向名称和来源元数据。Repository 只允许 `verified: true` 的 GCJ-02 坐标进入地图 marker。
...
7. 结合 2026 年站牌、通知和实际乘车复核线路数据；
8. 建立带别名的校园 POI 搜索；
9. 实现单线路直达推荐；
10. 接入上车前、下车后的步行路线；
11. 增加路线排序、时刻表和多方案；
12. 稳定后迁移可变数据到 CloudBase；
13. 仅在获得正式授权接口后接入实时车辆。
...
命令会逐条输出线路结果，并以退出码 `0` 表示全部通过、退出码 `1` 表示存在数据错误，可直接用于 CI。当前规则包括：所有 `stopId` 必须存在；普通线路不能重复经过同一站；环线必须首尾闭合，闭合站可以重复一次，其他重复站必须通过 `allowedRepeatedStopIds` 明确声明；线路至少包含一个方向；方向名称不能为空；每条线路必须在 `sources.json` 中具有非空的 `source` 和 `status`。
